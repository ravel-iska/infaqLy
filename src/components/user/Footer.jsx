...
export default function Footer() {
  ...
  return (
    <footer className="bg-slate-50 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 w-full rounded-t-[3rem] mt-auto transition-colors duration-300 shadow-2xl relative z-10 overflow-hidden">
      {/* Background Accents */}
      <div className="absolute top-0 inset-x-0 h-px bg-gradient-to-r from-transparent via-emerald-500/50 to-transparent"></div>
      <div className="absolute -top-24 -right-24 w-96 h-96 bg-emerald-500/5 dark:bg-emerald-500/10 rounded-full blur-[100px] pointer-events-none"></div>
      <div className="absolute -bottom-24 -left-24 w-96 h-96 bg-teal-500/5 dark:bg-teal-500/10 rounded-full blur-[100px] pointer-events-none"></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-10 lg:gap-14 px-8 lg:px-12 py-20 max-w-7xl mx-auto font-body text-sm leading-relaxed relative z-10">
        <div className="md:col-span-2 space-y-8">
          <div className="text-2xl font-headline font-bold text-emerald-800 dark:text-white flex items-center gap-2 transition-colors">
            <span className="material-symbols-outlined text-[32px] text-emerald-500 dark:text-emerald-400">volunteer_activism</span>
            Infaqly
          </div>
          <p className="text-slate-600 dark:text-slate-400 max-w-sm text-base leading-relaxed transition-colors">Membangun ekosistem filantropi digital yang amanah, transparan, dan berdampak luas bagi umat.</p>
          <div className="flex gap-4">
            <button
              ...
              className="w-12 h-12 rounded-full bg-white dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/50 flex items-center justify-center text-emerald-600 dark:text-emerald-400 cursor-pointer hover:bg-emerald-500 dark:hover:bg-emerald-500 hover:text-white dark:hover:text-white hover:border-transparent dark:hover:border-transparent transition-all shadow-lg hover:-translate-y-1"
            >
        ...
        <div className="space-y-6">
          <h5 className="font-bold text-slate-800 dark:text-white text-lg tracking-wide transition-colors">Program</h5>
          <ul className="space-y-4 text-slate-600 dark:text-slate-400 font-medium transition-colors">
            <li><Link to="/explore" className="hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors">Zakat Profesi</Link></li>
            <li><Link to="/explore" className="hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors">Infaq Pendidikan</Link></li>
            <li><Link to="/explore" className="hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors">Wakaf Produktif</Link></li>
            <li><Link to="/explore" className="hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors">Sedekah Pangan</Link></li>
          </ul>
        </div>

        <div className="space-y-6">
          <h5 className="font-bold text-slate-800 dark:text-white text-lg tracking-wide transition-colors">Bantuan</h5>
          <ul className="space-y-4 text-slate-600 dark:text-slate-400 font-medium transition-colors">
            <li>
              <a
            ...
            <li><Link to="/cara-donasi" className="hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors">Cara Donasi</Link></li>
            <li>
              <button onClick={() => setIsBugOpen(true)} className="hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors flex items-center gap-2 group">
                <span className="material-symbols-outlined text-[18px] group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">bug_report</span> Lapor Bug Panel
              </button>
            </li>
          </ul>
        </div>
        ...
        <div className="space-y-6">
          <h5 className="font-bold text-slate-800 dark:text-white text-lg tracking-wide transition-colors">Kontak</h5>
          <ul className="space-y-4 text-slate-600 dark:text-slate-400 font-medium transition-colors">
            <li className="flex items-center gap-3 group">
              <span className="material-symbols-outlined text-[20px] text-slate-400 dark:text-slate-500 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">mail</span>
              <a href="mailto:info@infaqly.org" className="hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors">info@infaqly.org</a>
            </li>
            {hasWa && (
              <li className="flex items-center gap-3 group">
                <span className="material-symbols-outlined text-[20px] text-slate-400 dark:text-slate-500 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">support_agent</span>
                <a href="/api/settings/whatsapp-redirect" target="_blank" rel="noopener noreferrer" className="hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors">Tanya via WhatsApp</a>
              </li>
            )}
            <li className="flex items-start gap-3 group">
              <span className="material-symbols-outlined text-[20px] text-slate-400 dark:text-slate-500 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors mt-[2px]">location_on</span>
              <a href="https://www.google.com/maps/search/?api=1&query=South+Quarter+Tower+A+Jakarta" target="_blank" rel="noopener noreferrer" className="hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors leading-relaxed">
      ...
      <div className="max-w-7xl mx-auto px-8 lg:px-12 pb-8 pt-8 border-t border-slate-200 dark:border-slate-800/80 flex flex-col md:flex-row justify-between items-center gap-6 text-slate-500 text-sm relative z-10 transition-colors duration-300">
        <p className="text-center md:text-left text-slate-500 dark:text-slate-500">© {new Date().getFullYear()} Infaqly Philanthropy. All rights reserved. <br className="md:hidden mt-2" /><span className="hidden md:inline"> | </span>Dibuat dengan <span className="text-red-500 mx-1">♥</span> oleh <span className="font-bold text-emerald-600 dark:text-emerald-400">Bagus Priambudi</span></p>
